...
impl Network {
    ...
    fn feedforward(&self, input: &Array1<f32>) -> Array1<f32> {
        let mut a: Array1<f32> = get_activation(input);
    ...
    fn backprop(
        ...
        let mut activation = get_activation(x);
...
fn get_activation(x: &Array1<f32>) -> Array1<f32> {
    let kers = kernels::get_kernels();
    let mut out = Array1::zeros(x.dim() + kers.len() * x.dim() / 16);
    let mut k = 0;
    for kernel in kers {
        let a = kernels::apply_kernel(x, 28, kernel);
        for m in a.iter() {
            out[k] = *m;
            k += 1;
        }
